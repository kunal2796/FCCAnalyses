// Studying the Spring2021 event files (Zcc): ghost matching - reco level (exclusive with exactly 2 jets [for now])
...
int main()
{
  ...
  TFile *file = TFile::Open("p8_ee_Zcc_ecm91_gm7x_auto.root");
  TTreeReader tree("events", file);
  int nEvents = tree.GetEntries();
  cout<<"Number of Events: "<<nEvents<<endl;

  TString histfname;
  histfname = "histZcc_gm7x_auto.root";
