...
plt.plot(tpr_u, fpr_u, label='up quarks', linestyle='-', color='b')
plt.plot(tpr_d, fpr_d, label='down quarks ', linestyle='-', color='r')
plt.plot(tpr_s, fpr_s, label='strange quarks ', linestyle='-', color='c')
...
plt.xlabel(r'Signal Efficiency ($\mathbf{\varepsilon_{sig}}$)')
plt.ylabel(r'Background Efficiency ($\mathbf{\varepsilon_{bkg}}$)')
...
plt.savefig('plots/LodeNet_filesplitter_log.png')
np.savez('ROC_params.npz', tpr_u, tpr_d, tpr_s, fpr_u, fpr_d, fpr_s, thresholds_u, thresholds_d, thresholds_s)
