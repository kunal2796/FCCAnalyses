...
P_s = np.nan_to_num(np.histogram(compute_P(p4)[strange_mask_cut], bins=P_bins)[0]/np.histogram(compute_P(p4)[strange_mask], bins=P_bins)[0])
Lumi = compute_lumi(len(jet_mask))
print('P_s')
print(P_s)
print(np.histogram(compute_P(p4)[strange_mask_cut], bins=P_bins)[0])
print(np.histogram(compute_P(p4), bins=P_bins)[0])
...
plt.plot(P_bins[:-1], P_s, label=r'strange jet efficiency ($\mathbf{\varepsilon_{sig}}$)', linestyle='', marker='x')
#plt.plot(P_bins[:-1], np.histogram(compute_P(p4)[strange_mask_cut], bins=P_bins)[0], label=r'strange jet efficiency ($\mathbf{\varepsilon_{sig}}$)', linestyle='', marker='x')
...
#plt.yscale('log')
plt.xlim(min(P_bins), max(P_bins))
plt.ylabel(r'Efficiency ($\mathbf{\varepsilon_{sig}})$')
plt.xlabel(r'Jet Momentum [GeV]')
...
plt.savefig('../plots/efficiencyP.pdf')
